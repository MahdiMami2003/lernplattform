...
<script lang="ts">
	...
	const staticPages = [
		...
		},
		{
			...
			keywords: 'register, signup, konto',
			subject: 'Seite',
			allowedRoles: null
		},
		{
			title: 'Lernmaterialien (Fächer)',
			url: '/material_page_id14',
			keywords: 'material, fächer, lernen',
			subject: 'Seite',
			allowedRoles: null
		},
		{
	...
	async function runSearch() {
		...
		// Removed school_class because of type mismatch issues in previous versions
...
<nav class="header-root">
	...
	<div class="icon-container">
		...
		{:else}
			<button class="login" onclick={goToLogin} style="cursor: default;"
				...
				{#if data.session}
					<li>
						<a href="/" onclick={handleLogout}
							><img alt="logout" src={login} />{$_('header.menu_logout')}</a
						>
					</li>
				{/if}
				{#if !data.session}
...
=======
        trans
			tion: background-color 0.3s
			ease, color 0.3s ease;
>>>>>>> f97d765 (Save local header changes before rebase)
    }
