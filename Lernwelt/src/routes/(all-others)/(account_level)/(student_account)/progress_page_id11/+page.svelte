...
<script lang="ts">
	...
	async function loadData(targetId: string) {
		try {
			...
			// 🛠️ MISSION SYNC: Prüfen, ob alle Missionen dem User zugewiesen sind
			const { data: allMissions } = await supabase.from('missions').select('id');

			if (allMissions && allMissions.length > 0 && isOwner && viewerRole === 'student') {
				// IDs die der User schon hat
				const existingIds = new Set(mData ? mData.map((x: any) => x.mission_id) : []);

				// IDs die fehlen
				const missing = allMissions.filter((m) => !existingIds.has(m.id));

				if (missing.length > 0) {
					console.log(`Füge ${missing.length} neue Missionen hinzu...`);
					...
					const { error: insError } = await supabase.from('missions_progress').insert(inserts);

					if (!insError) {
						// Daten neu laden, damit die neuen Missionen angezeigt werden
						const { data: reData } = await supabase
							...
							.order('completed', { ascending: true });
						missionsData = reData || [];
						return; // Fertig mit Reload
					} else {
						console.error('Fehler beim Syncen der Missionen:', insError);
					}
				}
			}

			// Falls kein Reload nötig war:
			missionsData = mData || [];
		} catch (err: any) {
			console.error(err);
						...
						{#if item.missions}
							<div class="card {item.completed ? 'done' : ''}">
								<div class="head">
									...
									<p class="mission-desc">{item.missions.description || ''}</p>
