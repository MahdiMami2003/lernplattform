<!--Lernwelt/src/routes/(all-others)/(account_level)/(student_account)/(tests + games)/deutsch_game/+page.svelte-->
<script lang="ts">
	...
	async function loadProfileAndQuestions() {
		try {
			...
			let query = supabase.from('questions').select('*').ilike('subject', 'Deutsch%');
			...
			if (error) {
				...
				console.warn('⚠ Keine Deutsch-Fragen gefunden.');
	...
	// 🔥 WICHTIG: Async gemacht, um Datenbank aufzurufen
	async function handleAnswerClick(index: number) {
		...
		if (isCorrect) {
			...
			// ------------------------------------------
			// 🚀 NEU: Mission Progress für 'Deutsch'
			// ------------------------------------------
			// ------------------------------------------
			// 🚀 NEU: Mission Progress für 'Deutsch'
			// ------------------------------------------
	...
	async function saveBossResult() {
		...
		await updateProfile({ last_boss: 'Classe10Deutsch', last_grade: grade });
				...
				<div class="achievements">
					...
					<div class="badge-row">
						...
						{#if !outOfHearts && correctCount >= Math.ceil(questions.length / 2)}
							<span class="badge">Deutsch-Profi 📘</span>
						{/if}
					</div>
				</div>
...
<style>
    ...
    .answer-btn:focus-visible {
        outline: 2px solid var(--text-primary, #1d5e84);
        outline-offset: 2px;
    }

    /* Feedback colors bleiben gleich in beiden Modi! */
    .answer-btn.correct {
        background: #c6f6d5;
        border-color: #3ba776;
        color: #065f46;
    ...
    /* XP Chest - bleibt golden in beiden Modi! */
    .xp-chest {
        position: relative;
        width: 130px;
        height: 100px;
    ...
    :root.darkmode .summary-stats strong {
        color: #4a9fc7;
    }
    ...
    :root.darkmode .summary-actions button:nth-child(2) {
        color: var(--text-primary, #e6edf3);
    }
    ...
    /* Feedback colors bleiben gleich! */
    ...
    .check-btn:disabled {
        background: var(--bg-hover, #cbd5e0);
        cursor: not-allowed;
        box-shadow: none;
        opacity: 0.6;
    }
