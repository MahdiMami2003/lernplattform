<!--Lernwelt/src/routes/(all-others)/(account_level)/(student_account)/(tests + games)/physik_game/+page.svelte-->
<script lang="ts">
	...
	async function loadProfileAndQuestions() {
		try {
			...
			if (error) {
				...
				loadError = 'Fehler beim Laden der Physik-Fragen.';
			} else if (!data || data.length === 0) {
				console.warn('⚠ Keine Physik-Fragen gefunden.');
	...
	async function handleAnswerClick(index: number) {
		...
		if (isCorrect) {
			...
			// -----------------------------------------------------------
			// 🚀 NEU: Mission Progress Update für 'Physik'
			// -----------------------------------------------------------
			// -----------------------------------------------------------
			// 🚀 NEU: Mission Progress Update für 'Physik'
			// -----------------------------------------------------------
